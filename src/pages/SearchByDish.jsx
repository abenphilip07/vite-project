...
const loadOptions = async (inputValue, callback) => {
  try {
    const res = await axios.get(`http://localhost:4000/v1/recipes${inputValue ? `?title=${inputValue}` : ''}`);
    let data;
...
const SearchByDish = () => {
  ...
  return (
    <Box p={5}>
      <Box mb={5}>
      <AsyncSelect cacheOptions defaultOptions={true} loadOptions={loadOptions} onChange={handleChange} />
            </Box>
      {selectedOption && (
